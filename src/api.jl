function quandl(id::String; rows=100, period="daily", format=TimeArray)
    ...
    elseif format == DataFrame 
        msg("DataFrame format not supported yet - please submit a PR \n 
             You can lift the timearray code and create a file named dataframe.jl \n 
             And then make replace this msg with dataframe(rq)") 
    else
        msg("wat? If you want this format, please submit a PR. :-)")
    end
