function quandl(id::String; order="des", rows=100, period="daily", format="TimeArray")
    ...
    if length(auth_token) >  50 
        rq = get( "http://www.quandl.com/api/v1/datasets/$id.csv?sort_order=$order&rows=$rows&collapse=$period") 
    else
        rq = get("http://www.quandl.com/api/v1/datasets/$id.csv?sort_order=$order&rows=$rows&collapse=$period&auth_token=$auth_token")
    end
